...
public abstract class Graph {
    ...
    public List<Integer> degreeSequence() {
        ...
        return IntStream.range(0, getNumVertices()).map(
                idx -> getNeighbors(idx).size() + getInNeighbors(idx).size()
    ...
    public static void main(String[] args) {
        //GraphLoader.createIntersectionsFile("data/maps/simpletest.map", "data/intersections/ucsd.intersections");
        ...
        System.out.println("Loading graphs based on real data. Goal: use degree sequence to analyse graphs. Roads/intersections:");
        ...
        for (int i = 0; i < listGraph.getNumVertices(); i++) {
            ...
            List<Integer> listTwoHopsVertices = listGraph.getDistance2(i);
            List<Integer> matrixTwoHOpVertices = matrixGraph.getDistance2(i);

            if (listTwoHopsVertices.equals(matrixTwoHOpVertices)) {
                System.out.println("Methods gave same results");
            } else {
                System.out.printf("Methods gave different results for vertices %s:\n" +
                                "\t   list graph result: %s\n" +
                                "\t matrix graph result: %s\n" +
                                "List graph: %s\n" +
                                "Matrix graph: %s\n",
                        i, listTwoHopsVertices, matrixTwoHOpVertices, listGraph, matrixGraph);
